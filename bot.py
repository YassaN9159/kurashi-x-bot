...
KURASHI_KEYWORDS = [
    # ① インテリア・寝具・収納
    "インテリア", "家具", "ソファ", "テーブル", "椅子", "棚", "本棚", "シェルフ",
    "照明", "ライト", "カーテン", "ブラインド", "ラグ", "マット", "クッション", "壁紙",
    "ベッド", "マットレス", "枕", "布団", "寝具", "掛け布団", "シーツ",
    "収納", "整理", "片付け", "断捨離", "整頓", "収納グッズ", "仕切り",
    "ボックス", "かご", "バスケット", "引き出し", "クローゼット", "押し入れ",
    "ハンガーラック", "突っ張り棒", "フック", "マグネット収納",
    # ② キッチン用品・食器・調理器具
    "キッチン", "食器", "調理器具", "調理家電",
    "食洗機", "電気ケトル", "ホットクック", "電子レンジ", "炊飯器",
    "コーヒーメーカー", "トースター", "オーブンレンジ", "ホットプレート",
    "フライパン", "鍋", "包丁", "まな板", "タッパー", "保存容器",
    "水筒", "タンブラー", "マグカップ", "グラス", "プレート",
    # ③ 日用品雑貨・文具・手芸
    ...
    # ブランド
    "無印良品", "IKEA", "イケア", "ニトリ", "カインズ", "山崎実業", "tower",
]
...
EXCLUDE_KEYWORDS = [
    ...
    "スマートフォン", "スマートウォッチ", "Apple Watch",
    # ゲーム
    "ゲーム", "PlayStation", "Nintendo", "Xbox", "ゲーミング",
    # 自動車
    "自動車", "クルマ", "バイク", "EV", "モーターショー", "SUV", "セダン",
    "アウディ", "BMW", "ベンツ", "トヨタ", "日産", "ボルボ", "VW", "フィアット",
    "ヤマハ", "ホンダ", "マツダ",
    # 金融・投資
    "株", "投資", "FX", "仮想通貨", "暗号資産", "NISA", "iDeCo",
    # エンタメ
    "アニメ", "マンガ", "映画", "ドラマ", "音楽", "ライブ",
    # 転職・スキル
    "転職", "就活", "資格", "副業",
    # カメラ・AV機器
    "カメラ", "レンズ", "ミラーレス", "一眼",
    "イヤホン", "ヘッドホン", "AirPods", "スピーカー",
    # PC周辺機器
    "GPU", "CPU", "SSD", "メモリ", "モニター", "キーボード", "マウス",
    # ファッション（アカウントテーマ外）
    "ファッション", "コーデ", "コーディネート", "ブランド品", "バッグ", "財布",
    "シューズ", "スニーカー", "ヒール",
    # ランキング・比較記事（特定商品に紐づけられないため除外）
    "おすすめ人気ランキング", "徹底比較", "人気ランキング", "おすすめランキング",
    "ランキング発表", "比較してみた", "選び方ガイド",
]
...
def _is_kurashi_by_claude(title: str, source: str) -> bool:
    ...
    prompt = (
        f"以下の記事タイトルが、下記のいずれかのカテゴリに関連するか判定してください。\n\n"
        f"対象カテゴリ：\n"
        f"① インテリア・寝具・収納（家具/照明/カーテン/ベッド/収納グッズ等）\n"
        f"② キッチン用品・食器・調理器具（調理家電/食器/鍋/フライパン等）\n"
        f"③ 日用品雑貨（掃除・洗濯・空調家電/アロマ/バス・トイレ用品等）\n"
        f"④ キッズ・ベビー・マタニティ（ベビー用品/子ども部屋/育児グッズ等）\n\n"
        f"タイトル: {title}\nソース: {source}\n\n"
        f"上記カテゴリに該当する場合は「YES」、該当しない場合は「NO」とだけ答えてください。"
    )
    try:
...
【取り扱いカテゴリ（このアカウントはこれのみ扱う）】
① インテリア・寝具・収納（家具/照明/ベッド/収納グッズ等）
② キッチン用品・食器・調理器具（調理家電/食器/鍋等）
③ 日用品雑貨（掃除・洗濯家電/アロマ/バス用品等）
④ キッズ・ベビー・マタニティ（ベビー用品/育児グッズ等）
...
- 文字数：200〜600 字（ハッシュタグ含む）。認証ユーザーなので長めに書いてOK。商品の特徴・使いどころ・おすすめポイントを具体的に書くこと
...
def post_article_to_x(article: dict, tweet_text: str, amazon_url: str, rakuten_url: str, image_data: bytes | None) -> None:
    ...
    try:
        ...
        if tweet_id:
            gw_line = f"\n🎌 GWセール → https://www.amazon.co.jp/events/monthlydealevent?tag={ASSOCIATE_ID}" if is_gw_sale_period() else ""
